...
class I18n {
    constructor(config) {
        this.config = Object.assign(Object.assign({}, defaultLocale), config);
    }
    findLabel(key) {
        if (this.config) {
...
const WIDTH_FORMAT = "width";
const HEIGHT_FORMAT = "height";
const ALIGN_FORMAT = "resizeAlign";
const ALT_FORMAT = "alt";
const TITLE_FORMAT = "title";
const VIDEO_FILE_BLOT_NAME = "videoFile";
...
function withResizeFormats(BaseBlot, attributors) {
    return class ResizableBlot extends BaseBlot {
        static formats(domNode) {
            const formats = typeof BaseBlot.formats === "function"
                ? Object.assign({}, BaseBlot.formats(domNode)) : {};
...
function syncResizeStateToQuill(quill, target) {
    var _a;
    if (!((_a = quill === null || quill === void 0 ? void 0 : quill.constructor) === null || _a === void 0 ? void 0 : _a.find) || typeof quill.getIndex !== "function") {
        return;
    }
...
class ResizePlugin {
     ...
     */
    _clampDimension(value, min, max) {
        const FLOOR = 30;
        let result = Math.max(value, Math.max(FLOOR, min !== null && min !== void 0 ? min : 0));
    ...
    _clampSize(width, height) {
        var _a;
        const constraints = ((_a = this.options) === null || _a === void 0 ? void 0 : _a.constraints) || {};
        return {
    ...
    applyToolbarVisibility() {
        ...
        const showToolbar = ((_a = this.options) === null || _a === void 0 ? void 0 : _a.showToolbar) !== false;
        const toolbarOptions = ((_b = this.options) === null || _b === void 0 ? void 0 : _b.toolbar) || {};
        ...
        const showAlignTools = (_d = (_c = toolbarOptions.alignTools) !== null && _c !== void 0 ? _c : toolbarOptions.alingTools) !== null && _d !== void 0 ? _d : true;
        const toolbar = this.resizer.querySelector(".toolbar");
        if (toolbar) {
            toolbar.style.display = showToolbar ? "" : "none";
        }
        const sizeGroup = this.resizer.querySelector('[data-group="size"]');
        if (sizeGroup) {
            sizeGroup.style.display = showSizeTools ? "" : "none";
        }
        const alignGroup = this.resizer.querySelector('[data-group="align"]');
        if (alignGroup) {
            alignGroup.style.display = showAlignTools ? "" : "none";
        }
        const showAttributesTool = toolbarOptions.attributesTool !== false;
        const attributesGroup = this.resizer.querySelector('[data-group="attributes"]');
        if (attributesGroup) {
            attributesGroup.style.display = showAttributesTool ? "" : "none";
        }
        const sizeLabel = this.resizer.querySelector(".size-label");
        if (sizeLabel) {
    ...
    _configureSizeToolbar() {
        ...
        const toolbarOptions = ((_a = this.options) === null || _a === void 0 ? void 0 : _a.toolbar) || {};
        const unit = (_b = toolbarOptions.sizeUnit) !== null && _b !== void 0 ? _b : "%";
        const presets = (_c = toolbarOptions.sizePresets) !== null && _c !== void 0 ? _c : [100, 50];
        const sizeGroup = this.resizer.querySelector('[data-group="size"]');
        if (sizeGroup) {
        ...
        const suffix = this.resizer.querySelector(".input-wrapper .suffix");
        if (suffix) {
            suffix.textContent = unit;
        }
        const input = this.resizer.querySelector('input[data-type="width"]');
        if (input) {
    ...
    positionResizerToTarget(el) {
        ...
        if (this.resizer !== null) {
            ...
            if ((_a = this.options) === null || _a === void 0 ? void 0 : _a.showSize) {
                const sizeLabel = this.resizer.querySelector(".size-label");
                if (sizeLabel) {
                    sizeLabel.textContent = `${Math.round(el.clientWidth)} x ${Math.round(el.clientHeight)}`;
                }
            }
    ...
    _syncPersistence() {
        var _a;
        const quill = (_a = this.options) === null || _a === void 0 ? void 0 : _a.__quillInstance;
        if (quill) {
     ...
     */
    _computeWidthStyles(percent) {
        var _a, _b, _c, _d;
        const unit = (_c = (_b = (_a = this.options) === null || _a === void 0 ? void 0 : _a.toolbar) === null || _b === void 0 ? void 0 : _b.sizeUnit) !== null && _c !== void 0 ? _c : "%";
        if (unit === "px") {
            const originSize = this.resizeTarget.originSize;
            const baseWidth = (originSize === null || originSize === void 0 ? void 0 : originSize.width) || this.resizeTarget.clientWidth || 0;
            const rawWidth = Math.round((baseWidth * percent) / 100);
            const constraints = ((_d = this.options) === null || _d === void 0 ? void 0 : _d.constraints) || {};
            const width = this._clampDimension(rawWidth, constraints.minWidth, constraints.maxWidth);
    ...
    toolbarInputChange(e) {
        var _a, _b, _c, _d, _e;
        const target = e.target;
        const type = (_a = target === null || target === void 0 ? void 0 : target.dataset) === null || _a === void 0 ? void 0 : _a.type;
        const value = Number(target.value);
        if (type && value) {
            const unit = (_d = (_c = (_b = this.options) === null || _b === void 0 ? void 0 : _b.toolbar) === null || _c === void 0 ? void 0 : _c.sizeUnit) !== null && _d !== void 0 ? _d : "%";
            if (unit === "px") {
                const constraints = ((_e = this.options) === null || _e === void 0 ? void 0 : _e.constraints) || {};
                const width = this._clampDimension(value, constraints.minWidth, constraints.maxWidth);
    ...
    toolbarClick(e) {
        ...
        const target = e.target;
        const type = (_a = target === null || target === void 0 ? void 0 : target.dataset) === null || _a === void 0 ? void 0 : _a.type;
        const action = (_b = target === null || target === void 0 ? void 0 : target.dataset) === null || _b === void 0 ? void 0 : _b.action;
        if (type === "attributes") {
        ...
        if (type && target.classList.contains("btn")) {
            const percentAttr = (_c = target.dataset) === null || _c === void 0 ? void 0 : _c.percent;
     ...
     */
    _toggleAttributesPanel(show) {
        var _a, _b;
        const panel = (_a = this.resizer) === null || _a === void 0 ? void 0 : _a.querySelector(".attributes-panel");
        const trigger = (_b = this.resizer) === null || _b === void 0 ? void 0 : _b.querySelector('[data-type="attributes"]');
        if (!panel || !trigger) {
            return;
        }
        const shouldShow = show !== null && show !== void 0 ? show : panel.hidden;
        if (shouldShow) {
            const isImage = this.resizeTarget.tagName.toLowerCase() === "img";
            const altField = panel.querySelector('[data-field="alt"]');
            if (altField) {
                altField.style.display = isImage ? "" : "none";
            }
            const altInput = panel.querySelector('input[data-attr="alt"]');
            if (altInput) {
                altInput.value = this.resizeTarget.getAttribute("alt") || "";
            }
            const titleInput = panel.querySelector('input[data-attr="title"]');
            if (titleInput) {
    ...
    _saveAttributes() {
        ...
        if (isImage) {
            const altInput = panel.querySelector('input[data-attr="alt"]');
            const alt = (_b = altInput === null || altInput === void 0 ? void 0 : altInput.value) !== null && _b !== void 0 ? _b : "";
            if (alt) {
        ...
        const titleInput = panel.querySelector('input[data-attr="title"]');
        const title = (_c = titleInput === null || titleInput === void 0 ? void 0 : titleInput.value) !== null && _c !== void 0 ? _c : "";
        if (title) {
...
function extractImageFiles(source) {
    ...
    for (let i = 0; i < source.length; i++) {
        ...
        const file = "getAsFile" in entry && typeof entry.getAsFile === "function"
            ? entry.getAsFile()
            : entry;
...
async function compressImage(file, options) {
    ...
    try {
        ...
        const quality = (_a = options.quality) !== null && _a !== void 0 ? _a : 0.8;
...
function normalizeYouTubeIframe(iframe) {
    ...
    const normalizedSrc = `https://www.youtube.com/embed/${videoId}` +
...
function resolveConstraints(tagName, options) {
    var _a;
    const perTag = (_a = options === null || options === void 0 ? void 0 : options.constraintsByTag) === null || _a === void 0 ? void 0 : _a[tagName];
    if (!(options === null || options === void 0 ? void 0 : options.constraints) && !perTag) {
        return undefined;
    }
    return Object.assign(Object.assign({}, options === null || options === void 0 ? void 0 : options.constraints), perTag);
}
/**
 ...
 */
async function insertUploadedImages(files, quill, options) {
    var _a, _b, _c, _d, _e, _f;
    for (const file of files) {
        const processed = await compressImage(file, options.imageCompression);
        const uploadFile = processed instanceof File
            ? processed
            : new File([processed], file.name, {
                type: processed.type || file.type,
            });
        const url = await ((_a = options.onImageUpload) === null || _a === void 0 ? void 0 : _a.call(options, uploadFile));
        if (!url) {
            continue;
        }
        const selection = (_b = quill.getSelection) === null || _b === void 0 ? void 0 : _b.call(quill, true);
        const index = selection ? selection.index : (_d = (_c = quill.getLength) === null || _c === void 0 ? void 0 : _c.call(quill)) !== null && _d !== void 0 ? _d : 0;
...
function resolveClickTarget(clickedTarget, event, options) {
    var _a, _b, _c;
    const resolved = (_a = options === null || options === void 0 ? void 0 : options.resolveEmbed) === null || _a === void 0 ? void 0 : _a.call(options, clickedTarget, event);
    if (resolved) {
        return resolved;
    }
    const embedTags = (_b = options === null || options === void 0 ? void 0 : options.embedTags) !== null && _b !== void 0 ? _b : DEFAULT_EMBED_TAGS;
    const tagName = (_c = clickedTarget === null || clickedTarget === void 0 ? void 0 : clickedTarget.tagName) === null || _c === void 0 ? void 0 : _c.toLowerCase();
...
function QuillResizeModule(quill, options) {
    ...
    const pluginOptions = Object.assign(Object.assign({}, options), { __quillInstance: quill });
    const onContainerClick = (e) => {
        const clickedTarget = e.target;
        const target = resolveClickTarget(clickedTarget, e, options);
        if (target) {
            resizeTarge = target;
            resizePlugin = new ResizePlugin(target, container.parentElement, Object.assign(Object.assign({}, pluginOptions), { constraints: resolveConstraints(target.tagName.toLowerCase(), options) }));
        }
    };
    ...
    const onImagePaste = (e) => {
        ...
        const files = extractImageFiles((_a = e.clipboardData) === null || _a === void 0 ? void 0 : _a.files);
        if (files.length === 0) {
            return;
        }
        e.preventDefault();
    ...
    const onImageDrop = (e) => {
        ...
        const files = extractImageFiles((_a = e.dataTransfer) === null || _a === void 0 ? void 0 : _a.files);
        if (files.length === 0) {
            return;
        }
        e.preventDefault();
    ...
    const onTextChange = (_delta, _oldDelta, _source) => {
        ...
        container.querySelectorAll("iframe").forEach((item) => {
            ...
            IframeClick.track(item, () => {
                resizeTarge = item;
                resizePlugin = new ResizePlugin(item, container.parentElement, Object.assign(Object.assign({}, pluginOptions), { constraints: resolveConstraints("iframe", options), 
    ...
    return {
        ...
        destroy() {
            ...
            resizePlugin = null;
            resizeTarge = null;
            trackedIframes.forEach((iframe) => IframeClick.untrack(iframe));
            trackedIframes.clear();
        },
    };
}

export { QuillResizeModule as default };
//# sourceMappingURL=quill-resize-module.esm.mjs.map
